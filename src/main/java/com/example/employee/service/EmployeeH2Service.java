...
public class EmployeeH2Service implements EmployeeRepository {
    ...
    public Employee getEmployeeById(int employeeId) {
        try {
            Employee employee = db.queryForObject("selsct * from EMPLOYEELIST where employeeId =?", new EmployeeRowMapper(), employeeId);
            return employee;
        } catch (Exception e) {
    ...
    public Employee addEmployee(Employee employee) {
        ...
        Employee savedEmployee = db.queryForObject(
                "select * from EMPLOYEELIST where employeeName = ? and email = ? and depaetment =?",
